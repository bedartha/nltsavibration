...
def _load_indices():
    ...
    # load data
    enso = np.load(DATPATH + "nino34_long.npz")
    pdo = np.load(DATPATH + "pdo.npz")
    t_enso, x_enso = enso["t"], enso["x"]
    t_pdo, x_pdo = pdo["t"], pdo["x"]
...
def _get_det():
    ...
    FN = DATPATH + "det_WS%d_SS%d_EPS%dpc_LMIN%d_NSURR%d" \
                   % (ws, ss, EPS, LMIN, ns)
    np.savez(FN, DET=DET, tm=tm, DETSURR=DETSURR, SURR=SURR)
    if args.verbose: print("output saved to: %s.npz" % FN)
...
def _get_spl():
    ...
    FN = DATPATH + "spl_WS%d_SS%d_EPS%dpc_LMIN%d_NSURR%d" \
                   % (ws, ss, EPS, LMIN, ns)
    np.savez(FN, SPL=SPL, tm=tm, SPLSURR=SPLSURR, SURR=SURR)
    if args.verbose: print("output saved to: %s.npz" % FN)
...
def _get_cpr():
    ...
    # # plot check
    # hi = np.percentile(cprsurr, 99., axis=0)
    # # pl.fill_between(tm,  rmd, hi, where=rmd < hi, alpha=0.5)
    # pl.plot(tm, hi, "k--")
    # pl.plot(tm, cpr, "r-")
    # pl.show()
    # sys.exit()
    ...
    FN = DATPATH + "cpr_WS%d_SS%d_EPS%dpc_NSURR%d" \
                   % (ws, ss, EPS, ns)
    np.savez(FN, cpr=cpr, tm=tm, cprsurr=cprsurr, SURR=SURR)
    if args.verbose: print("output saved to: %s.npz" % FN)
...
def _get_rmd():
    ...
    # # plot check
    # hi = np.percentile(rmdsurr, 99., axis=0)
    # # pl.fill_between(tm,  rmd, hi, where=rmd < hi, alpha=0.5)
    # pl.plot(tm, hi, "k--")
    # pl.plot(tm, rmd, "r-")
    # pl.show()
    ...
    FN = DATPATH + "rmd_WS%d_SS%d_EPS%dpc_NSURR%d" \
                   % (ws, ss, EPS, ns)
    np.savez(FN, rmd=rmd, tm=tm, rmdsurr=rmdsurr, SURR=SURR)
    if args.verbose: print("output saved to: %s.npz" % FN)
...
def _get_communities():
    ...
    FN = DATPATH + "communities_EPS%d" \
                   % EPS
    np.savez(FN, x=x, t=t, COMM=COMM, m=m, tau=tau, e=EPS, thrby=thrby)
    if args.verbose: print("output saved to: %s.npz" % FN)
...
def _get_fig():
    ...
    ax.set_yticks(np.arange(0.50, 0.91, 0.20))
    ...
    # SPL
    ax = axes[1]
    ax.set_ylim(1.10, 2.50)
    ax.set_yticks(np.arange(1.40, 2.31, 0.30))
    ax.legend(loc="upper left")
    ...
    # RMD
    ax_.set_ylim(-3, 18)
    ax_.set_yticks(np.arange(0., 15.01, 5.))
    ...
    for ax in fig.axes:
        ...
        ax.tick_params(which="minor", size=3., top="on", bottom="on")
        if ax is not axes[4]:
